...
data Const
  ...
  | Float !Float   -- ^ [fpval]
  | Aggregate ![Word64] -- ^ [n x value number]
  | String !String
  | CString !String
  | BinOp !Word64 !Word64 !Word64 -- ^ [opcode, opval, opval]
  | Cast !Word64 !Word64 !Word64  -- ^ [opcode, opty, opval]
...
data Value
  ...
  = Global
    ...
    , gInitId :: Maybe Word64         -- ^ If non-zero, the value index of the initializer for this variable, plus 1.
    , gLinkage :: Linkage
    , gParamAttrs :: Word64
--    , gAlignment :: Word64          -- ^ The logarithm base 2 of the variable's requested alignment, plus 1
  ...
  | Constant Ty Const                  -- ^ constant values
